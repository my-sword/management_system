...
// 主窗体菜单栏
//ToolBar(getFrameMenuBar()→MenuBar)→→→→→→→→
import java.awt.*;
...
public class MenuBar extends JMenuBar {
	...
	public MenuBar(JDesktopPane desktopPanel, JLabel label) {
		super();
		iFrames = new HashMap<JMenuItem, JInternalFrame>();//哈希集合 字典<K,V>  定义菜单项对应的窗体集合
	 ...
	 */
	private JInternalFrame createIFrame(JMenuItem item, Class clazz) {//JInternalFrame本类的成员变量 Class继承JFrame类
		Constructor constructor = clazz.getConstructors()[0];	//获取第一个构造器
		JInternalFrame iFrame = iFrames.get(item);				//获取当前菜单项对应的窗体，将之赋值给本方法变量（空值）
		try {
			if (iFrame == null || iFrame.isClosed()) {			//如果窗体被关闭或者为空
				//new关键字能调用任何构造方法。newInstance()只能调用无参构造方法。
				iFrame = (JInternalFrame) constructor.newInstance(new Object[] {});//对构造器窗体进行构造调用
				iFrames.put(item, iFrame);						//将菜单项和窗体加入 集合
				iFrame.setFrameIcon(item.getIcon());			//调用方法getIcon()同步菜单项窗体图标
				iFrame.setLocation(nextFrameX, nextFrameY);		//调整位置（默认为0，即为桌面面板的左上角）
				//当组件不在可视范围(已经渲染完成)时,getHeight() 返回0  getPreferredSize() 则不会返回0,而是真实的高度.

				//第一个后开始，后发性实现层叠效果（第二个窗体的位置是建立在第一个方法之后）
				int frameH = iFrame.getPreferredSize().height;	//获取当前弹出窗体高度（不是位置）
				int panelH = iFrame.getContentPane().getPreferredSize().height;//获取组件在桌面面板底部的高度
				int fSpacing = frameH - panelH;					//叠加高度=底部的高度-弹出窗体高度
				nextFrameX += fSpacing;
				nextFrameY += fSpacing;
				if (nextFrameX + iFrame.getWidth() > desktopPanel.getWidth())//限制新窗体位置范围（右底超过为0）
					nextFrameX = 0;
				if (nextFrameY + iFrame.getHeight() > desktopPanel.getHeight())
					nextFrameY = 0;
				desktopPanel.add(iFrame);						//将窗体添加到桌面面板
			...
			iFrame.addInternalFrameListener(new InternalFrameAdapter() {
				public void internalFrameActivated(InternalFrameEvent e) {
					//super.internalFrameActivated(e);//调用该类的上类的internalFrameActivated(e)方法(无区别)
					JInternalFrame frame = e.getInternalFrame();//？？获取e所在类的对象
	...
	public JMenu getJinhuo_Menu() {
		if (jinhuo_Menu == null) {
			...
			jinhuo_Menu.setMnemonic(KeyEvent.VK_J); //Alt+J
			jinhuo_Menu.add(getJinhuoItem());		//调用菜单项1自类
			jinhuo_Menu.add(getJinhuo_tuihuoItem());//调用菜单项2自类
		}
		return jinhuo_Menu;
	}
	/**
	...
	public JMenuItem getJinhuoItem() {
		if (jinhuoItem == null) {//jinhuoItem次级菜单
			...
			jinhuoItem.addActionListener(new ActionListener() {
				...
				//调用自类(菜单项，菜单弹出窗体JinHuoDan_IFrame自类)
			});
		}
		return jinhuoItem;
	}
	...
	public JMenu getChuang_kouMenu() {
		if (chuang_kouMenu == null) {//目的：将桌面面板已存在窗口添加至窗口菜单之中
			...
			chuang_kouMenu.addMenuListener(new MenuListener() {
				public void menuSelected(MenuEvent e) {
					...
					for (final JInternalFrame frame : allFrames) {
						...
						item.addActionListener(new ActionListener() {// 窗体菜单项添加单击事件
							public void actionPerformed(ActionEvent e) {
								try {
									frame.setIcon(false);
									frame.setSelected(true);//窗体获得焦点
		...
		if (pingpuItem == null) {
			...
			pingpuItem.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					JInternalFrame[] allFrames = desktopPanel.getAllFrames();
					int x = 0, y = 0;
					for (JInternalFrame iFrame : allFrames) {//JInternalFrame是成员方法类型
						iFrame.setLocation(x, y);			 //根据集合顺序重新布局位置
						try {
							iFrame.setSelected(true);
						} catch (PropertyVetoException e1) {
							e1.printStackTrace();
			...
			closeAllItem.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					for (JInternalFrame frame : allFrames) {
						frame.doDefaultCloseAction();//以默认方式关闭窗口（不影响其他线程）
					}
				}
			});
		}
	...
	private JMenuItem getGuanyu_Item() {
		if (guanyu_Item == null) {
			...
			imgLabel.setVisible(false);			//创建主窗体即存在
			...
			guanyu_Item.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					imgLabel.setBounds((dw - 500) / 2, (dh - 350) / 2, 500, 350);//设置面板居中宽高
					imgLabel.setVisible(true);	//菜单单击触发显示
				}
			});
			...
			bugItem.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (Desktop.isDesktopSupported()) {
						Desktop desktop = Desktop.getDesktop();//获取系统桌面方法支持
						try {
							URI uri = new URI("mailto:tmoonbook@sina.com");
							desktop.mail(uri);//打开系统邮件（网页）
						} catch (Exception e1) {
							e1.printStackTrace();
						}
					}
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					if (Desktop.isDesktopSupported()) {
						...
						try {
							...
							desktop.browse(url.toURI());//打开默认浏览器网页url地址
						} catch (Exception e1) {
							e1.printStackTrace();
						}
					}
